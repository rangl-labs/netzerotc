...
class State:
    ...
    def to_observation(self):
        observation = (self.step_count,) + tuple(self.IEV_years)
...
def action_space():
    act_low = np.zeros(
        2 * param.scenarios, dtype=np.float32
    )  # scenario weights and scenario years to advance
...
def apply_action(action, state):
    ...
    for scenario in np.arange(param.scenarios):
        if (
            ...
            scenarioYears[scenario] = (
                param.steps_per_episode - 1 - state.IEV_years[scenario]
            )
    ...
    for scenario in np.arange(param.scenarios):  # for each scenario
        ...
        IEV_year = (
            state.IEV_years[scenario] + scenarioYears[scenario]
        )  # identify the last IEV year to be implemented this time
        for rewardType in np.arange(
            1, param.reward_types
    ...
    )  # record the latest IEV year implemented
...
def verify_constraints(state):
    ...
    if state.step_count > 1:
        if (
            state.weightedRewardComponents_all[-1][3]
            - state.weightedRewardComponents_all[-2][3]
            < -25000
        ):
            verify = False
    ...
    if state.step_count > 2:
        if (
            state.weightedRewardComponents_all[-1][3]
            - state.weightedRewardComponents_all[-3][3]
            < -37500
        ):
            verify = False
    return verify
