...
class Envs(object):
    ...
    def score(self, instance_id):
        env = self._lookup_env(instance_id)
        return env.score()

    def seed(self, instance_id, seed=None):
        env = self._lookup_env(instance_id)
        env.seed(seed)
        # nothing is returned
...
@app.route("/v1/envs/<instance_id>/score/", methods=["GET"])
def env_score(instance_id):
    """
    Fetch episode score
    """
    score = envs.score(instance_id)
    return jsonify(score=score)


@app.route("/v1/envs/<instance_id>/is_done/", methods=["GET"])
def env_is_done(instance_id):
    """
    look up the value of done
    """
    done = envs.is_done(instance_id)
    # return done - not allowed because
    # The return type must be a string, dict, tuple
    return jsonify(done=done)  # is possible or:
    # return str(done)
...
@app.route("/v1/envs/<instance_id>/seed/", methods=["POST"])
def env_seed(instance_id):
    """
    ...
    Parameters:
        - instance_id: a short identifier (such as '3c657dbc')
        for the environment instance
        - seed: a seed to be given to the environment
    Returns:
        - nothing
    """
    json = request.get_json()
    seed = get_required_param(
        json, "seed"
    )  # not sure what data structure is received from request.get_json()
    envs.seed(instance_id, seed)
...
#         for the environment instance
#     Returns:
#         - a score (a correct value when done==True and None when done==False)
#     """
#     score = envs.score(instance_id)
#     return score
# #NEW end
