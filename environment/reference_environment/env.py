...
class Parameters:
    ...
    reward_types = (
        6  # capex first, then opex, revenue, emissions, jobs, total economic impact
    )
    steps_per_episode = (
        30  # number of years in the planning horizon (eg. 2021 -> 2050 = 30)
    )
    IEV_Rewards = np.ones(
        (scenarios, steps_per_episode, reward_types)
    )  # rewards for each scenario in each year in the IEV model, by reward type (capex first, then opex, revenue, ...)
    IEV_RewardSensitivities = np.full_like(
        IEV_Rewards, 1.01
    )  # in any strategy, when any investment is brought forward by one year, its associated rewards must be multiplied by this factor to account for changing costs/reevenues over time
    IEV_RewardDerivatives = np.full_like(
        IEV_Rewards, 1.01
    )  # only the last 2 reward types (jobs, total economic impact) will be used
    ...
    IEV_Rewards[:, :, 0] = np.array(
        ...
        dtype=np.float64,
    )  # [:,4:] corresponds to 2021 -> 2050, 30 values/steps in total
    ...
    IEV_Rewards[:, :, 4] = np.array(
        ...
        dtype=np.float64,
    )  # for jobs only, [:,2:] corresponds to 2021 -> 2050, 30 values/steps in total
    ...
    IEV_RewardSensitivities[:, 0:-1, 0] = np.genfromtxt(
        "./sensitivities/Pathways to Net Zero - Total capex Sensitivity Ratio for Real 1-Year Acceleration No Wave+Tidal.csv",
        delimiter=",",
    )  # 0:-1 corresponds to 2021 -> 2049
    IEV_RewardSensitivities[:, 0:-1, 1] = np.genfromtxt(
        "./sensitivities/Pathways to Net Zero - Total opex Sensitivity Ratio for Real 1-Year Acceleration No Wave+Tidal.csv",
        delimiter=",",
    )
    IEV_RewardSensitivities[:, 0:-1, 2] = np.genfromtxt(
        "./sensitivities/Pathways to Net Zero - Total revenue Sensitivity Ratio for Real 1-Year Acceleration No Wave+Tidal.csv",
        delimiter=",",
    )
    IEV_RewardSensitivities[:, 0:-1, 3] = np.genfromtxt(
        "./sensitivities/Pathways to Net Zero - Total emissions (uncaptured carbon tax) Sensitivity Ratio for Real 1-Year Acceleration No Wave+Tidal.csv",
        delimiter=",",
    )
    IEV_RewardSensitivities[:, 0:-1, 4] = np.genfromtxt(
        "./sensitivities/IEV - Total Jobs Sensitivity Ratio for Real 1-Year Acceleration No Wave+Tidal.csv",
        delimiter=",",
    )  # including direct and indirect, but without induced; may read the "IEV - Total Jobs Including Induced Sensitivity Ratio for 1-Year Shifting.csv" to load the sensitivity for induced jobs included.
    IEV_RewardSensitivities[:, 0:-1, 5] = np.genfromtxt(
        "./sensitivities/IEV - Total Economic Impact Sensitivity Ratio for Real 1-Year Acceleration No Wave+Tidal.csv",
        delimiter=",",
    )
    IEV_RewardDerivatives[:, :, 4] = np.genfromtxt(
        "./sensitivities/IEV - Total Jobs Derivative for capex+Delta100.csv",
        delimiter=",",
    )
    IEV_RewardDerivatives[:, :, 5] = np.genfromtxt(
        "./sensitivities/IEV - Total Economic Impact Derivative for capex+Delta100.csv",
        delimiter=",",
    )
...
class State:
    ...
    def initialise_state(self):
        ...
        self.step_count = -1
        self.IEV_years = np.zeros(
            param.scenarios, dtype=int
        )  # for each scenario, records the latest IEV year that has been implemented
        self.jobs = np.float32(
            139964
        )  # initial jobs of year 2020, extracted from the IEV model spreadsheet
...
def observation_space(self):
    ...
    obs_low[
        ...
        -37500
    )  # last entry of obervation is the increment in jobs; Constraint 2: no decrease in jobs in excess of 37,500 per two years
    ...
    obs_high[-2] = (
        10 * 139964
    )  # 2nd last entry of obervation is the jobs; 10 times initial jobs in 2020 = 10*139964, large enough
    obs_high[
        -1
    ] = 139964  # last entry of obervation is the increment in jobs; jobs should can't be doubled in a year or increased by the number of total jobs in 2020
...
def action_space():
    act_low = np.concatenate(
        (
            np.zeros(param.scenarios, dtype=np.float32),
            np.ones(param.scenarios, dtype=np.float32),
        )
    )
...
def apply_action(action, state):
    ...
    for scenario in np.arange(param.scenarios):
        if (
            state.IEV_years[scenario] + scenarioYears[scenario]
            > param.steps_per_episode
        ):  # as discussed at https://rangl.slack.com/archives/D022M4WD4Q6/p1631892073001100?thread_ts=1631728756.006200&cid=D022M4WD4Q6 and https://rangl.slack.com/archives/D022M4WD4Q6/p1631892369001300?thread_ts=1631728756.006200&cid=D022M4WD4Q6
            scenarioYears[scenario] = (
                param.steps_per_episode - state.IEV_years[scenario]
            )
    # capex = 0 # this variable will aggregate all (rebased) capital expenditure for this time step
    rewardComponents = np.zeros(
        (param.scenarios, param.reward_types)
    )  # for each scenario, this array will hold all components of reward for this time step
    ...
    for scenario in np.arange(param.scenarios):  # for each scenario
        for IEV_year in np.arange(
            state.IEV_years[scenario],
            state.IEV_years[scenario] + scenarioYears[scenario],
        ):  # for each IEV year to be implemented this time
            IEV_year = int(IEV_year)
            IEV_YearReward = param.IEV_Rewards[
                scenario, IEV_year, 0
            ]  # get the raw capex
            for sensitivityYear in np.arange(state.IEV_years[scenario], IEV_year):
                IEV_YearReward *= param.IEV_RewardSensitivities[
                    scenario, sensitivityYear, 0
                ]  # apply each relevant sensitivity
        ...
        IEV_year = int(
            state.IEV_years[scenario] + scenarioYears[scenario] - 1
        )  # the last IEV year to be implemented
        IEV_YearReward = param.IEV_Rewards[
            scenario, IEV_year, 0
        ]  # get the raw capex of the last IEV year to be implemented
        for sensitivityYear in np.arange(state.IEV_years[scenario], IEV_year):
            IEV_YearReward *= param.IEV_RewardSensitivities[
                scenario, sensitivityYear, 0
            ]
        ...
        IEV_YearJobs = (
            param.IEV_Rewards[scenario, IEV_year, 4]
            + (IEV_YearCapex - param.IEV_Rewards[scenario, IEV_year, 0])
            * param.IEV_RewardDerivatives[scenario, IEV_year, 4]
        )
        IEV_YearEconoImpact = (
            param.IEV_Rewards[scenario, IEV_year, 5]
            + (IEV_YearCapex - param.IEV_Rewards[scenario, IEV_year, 0])
            * param.IEV_RewardDerivatives[scenario, IEV_year, 5]
        )
        ...
        for sensitivityYear in np.arange(state.step_count, IEV_year):
            ...
            IEV_YearEconoImpact *= param.IEV_RewardSensitivities[
                scenario, sensitivityYear, 5
            ]
        ...
        IEV_year = (
            state.IEV_years[scenario] + scenarioYears[scenario] - 1
        )  # identify the last IEV year to be implemented (*implementation starts from state.IEV_years inclusive, so there should be a -1 at the end*) this time (i.e., this corrent state.step_count)
        for rewardType in np.arange(
            1, param.reward_types - 2
        ):  # for each remaining reward type (these should all represent annual rates rather than one-off charges/rewards, and by convention we apply the last rate)
            IEV_year = int(IEV_year)
            IEV_YearRate = param.IEV_Rewards[
                scenario, IEV_year, rewardType
            ]  # get the raw reward rate
            for sensitivityYear in np.arange(state.step_count, IEV_year):
                IEV_YearRate *= param.IEV_RewardSensitivities[
                    scenario, sensitivityYear, rewardType
                ]  # apply each relevant sensitivity
            rewardComponents[scenario, rewardType] = IEV_YearRate
    weightedRewardComponents = np.matmul(
        scenarioWeights, rewardComponents
    )  # weight the reward components by the scenario weights
    state.IEV_years = np.clip(
        state.IEV_years + scenarioYears, 0, param.steps_per_episode - 1
    )  # record the latest IEV year implemented (to be implemented as the 1st year in the next step)
...
def plot_episode(state, fname):
    ...
    plt.subplot(222)
    plt.plot(
        np.array(state.observations_all)[:, :4]
    )  # first 4 elements of observations are step counts and 3 IEV years
