...
deployments = np.array(
    np.array(pd.read_excel("BREEZE_Deployments.xlsx"))[
        -(env.param.steps_per_episode + 1) :, 1:
    ],
    dtype=np.float32,
)
...
env.reset()
deployments = np.array(
    np.array(pd.read_excel("GALE_Deployments.xlsx"))[
        -(env.param.steps_per_episode + 1) :, 1:
    ],
    dtype=np.float32,
)
...
env.reset()
deployments = np.array(
    np.array(pd.read_excel("STORM_Deployments.xlsx"))[
        -(env.param.steps_per_episode + 1) :, 1:
    ],
    dtype=np.float32,
)
